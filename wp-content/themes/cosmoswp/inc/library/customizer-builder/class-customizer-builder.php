...
if( !function_exists( 'customizer_builder_sanitize_field_recursive')){

	function customizer_builder_sanitize_field_recursive( $value ){
		if (!is_array($value)) {
			...
			if (is_array($value)) {
				foreach ($value as $k => $v) {
					$value[$k] = customizer_builder_sanitize_field_recursive($v);
				}
			}
		}
		return $value;
...
if( !function_exists( 'customizer_builder_sanitize_field')){

	function customizer_builder_sanitize_field( $input ) {
		$input = wp_unslash( $input );
		if ( ! is_array( $input ) ) {
			$input = json_decode( urldecode_deep( $input ), true );
		}
		$output = customizer_builder_sanitize_field_recursive($input);
		return $output;
	}
}
