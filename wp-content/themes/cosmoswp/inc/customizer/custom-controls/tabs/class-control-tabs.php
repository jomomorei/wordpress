				...
				foreach( $tabs_and_fields as $tab_id => $tab_fields ){
					...
					foreach( $tab_fields as $key => $field ){
						...
						switch ( $field['type'] ) {
							...
							case 'color':
								echo '<div class="customize-control-alpha-color" data-single-name="' . esc_attr( $key ) . '"><input class="alpha-color-control" data-default="' . esc_attr( $default ) . '" data-single-name="' . esc_attr( $key ) . '" type="text" value="' . esc_attr( $new_value ) . '"/></div>';
								break;
